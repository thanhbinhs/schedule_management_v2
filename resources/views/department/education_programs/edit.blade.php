...
<div class="container mt-4">
    <h2 class="text-center mb-4">Sửa thông tin chương trình đào tạo</h2>
    ...
    <form action="{{ route('department.education_programs.update', $educationProgram->EducationProgramID) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="form-group mb-3">
            <label for="EducationProgramName" class="form-label"><i class="fas fa-graduation-cap me-1"></i> Chương trình đào tạo</label>
            <input type="text" name="EducationProgramName" class="form-control" id="EducationProgramName"
                value="{{ old('EducationProgramName', $educationProgram->EducationProgramName) }}" required>
        </div>

        <div class="form-group mb-3">
            <label for="MajorID" class="form-label"><i class="fas fa-award me-1"></i> Ngành học</label>
            <select name="MajorID" id="MajorID" class="form-control" required>
                <option value="" disabled>-- Chọn ngành học --</option>
                @foreach($majors as $major)
                    <option value="{{ $major->MajorID }}" {{ (old('MajorID', $educationProgram->MajorID) == $major->MajorID) ? 'selected' : '' }}>
                        {{ $major->MajorName }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group mb-3">
            <label for="SubjectList" class="form-label"><i class="fas fa-book-open me-1"></i> Môn học</label>
            <select name="SubjectList[]" id="SubjectList" class="form-control" multiple required>
                @foreach($subjects as $subject)
                    <option value="{{ $subject->SubjectID }}"
                        {{ (collect(old('SubjectList', $selectedSubjects))->contains($subject->SubjectID)) ? 'selected' : '' }}>
                        {{ $subject->SubjectName }}
                    </option>
                @endforeach
            </select>
            <small class="text-muted"><i class="fas fa-info-circle me-1"></i> Chọn nhiều môn học bằng cách giữ phím Ctrl (Windows) / Command (Mac).</small>
        </div>

        <button type="submit" class="btn btn-primary"><i class="fas fa-save me-2"></i> Cập nhật</button>
        <a href="{{ route('department.education_programs.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left me-2"></i> Trở về</a>
    </form>
</div>
@endsection
