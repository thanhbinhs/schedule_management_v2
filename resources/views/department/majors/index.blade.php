...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="mb-0 text-center">Thông tin ngành học</h2>
        <a href="{{ route('department.majors.create') }}" class="btn btn-primary"><i class="fas fa-plus-circle me-2"></i> Thêm mới</a>
    </div>

    @if($majors->isEmpty())
        <div class="alert alert-info"><i class="fas fa-info-circle me-2"></i> Không tìm thấy ngành học</div>
    @else
                    ...
                    @foreach($majors as $major)
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="{{ route('department.majors.edit', $major->MajorID) }}" class="btn btn-sm btn-warning"><i class="fas fa-edit me-1"></i> Sửa</a>
                                <form action="{{ route('department.majors.destroy', $major->MajorID) }}" method="POST" class="d-inline"
                                      onsubmit="return confirm('Bạn có chắc muốn xóa ngành học này?');">
                                    @csrf
                                    @method('DELETE')
