...
@section('head')
    ...
    <style>
        ...
        .btn-success {
            background-color: var(--primary-color); /* Changed to primary for consistency, or keep success if preferred */
            border-color: var(--primary-color); /* Changed to primary for consistency */
            color: var(--text-color-light);
        }

        .btn-success:hover {
            background-color: #0056b3; /* Hover matches primary hover now */
            border-color: #0056b3;
            transform: translateY(-2px);
            box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2);
        }
...
<div class="container mt-4">
    <h2 class="text-center mb-4">Thêm mới môn học</h2>
    ...
    <form action="{{ route('department.subjects.store') }}" method="POST">
        ...
        <div class="form-group mb-3">
            <label for="SubjectName" class="form-label"><i class="fas fa-book me-1"></i> Môn học</label>
        ...
        <div class="form-group mb-3">
            <label for="SubjectCredit" class="form-label"><i class="fas fa-coins me-1"></i> Số tín chỉ</label>
            <input type="number" name="SubjectCredit" class="form-control" id="SubjectCredit" value="{{ old('SubjectCredit') }}" min="0" required>
        </div>

        <div class="form-group mb-3">
            <label for="SubjectLessons" class="form-label"><i class="fas fa-hourglass-half me-1"></i> Số tiết</label>
            <input type="number" name="SubjectLessons" class="form-control" id="SubjectLessons" value="{{ old('SubjectLessons') }}" min="0" required>
        </div>

        <button type="submit" class="btn btn-primary"><i class="fas fa-plus-circle me-2"></i> Thêm mới</button>
        <a href="{{ route('department.subjects.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left me-2"></i> Trở về</a>
    </form>
</div>
@endsection