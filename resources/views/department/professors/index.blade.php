...
@section('head')
    ...
    <style>
        ...
        .table tbody tr.highlight-leader {
            background-color: #e0f7fa; /* Light cyan/teal for leader highlight */
        }

        .table tbody tr.highlight-leader:hover {
            background-color: #ccecef; /* Darker shade on hover for highlighted row */
        }
...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="mb-0 text-center">Thông tin giảng viên</h2>
        <a href="{{ route('department.professors.create') }}" class="btn btn-primary"><i class="fas fa-plus-circle me-2"></i> Thêm mới</a>
    </div>

    @if($professors->isEmpty())
        <div class="alert alert-info"><i class="fas fa-info-circle me-2"></i> Không tìm thấy giảng viên</div>
    @else
                    ...
                    @foreach($professors as $professor)
                        <tr class="{{ $professor->isLeaderDepartment ? 'highlight-leader' : '' }}">
                            ...
                            <td class="text-center">
                                <a href="{{ route('department.professors.edit', $professor->ProfessorID) }}" class="btn btn-sm btn-warning btn-action"><i class="fas fa-edit me-1"></i> Sửa</a>
                                <form action="{{ route('department.professors.destroy', $professor->ProfessorID) }}" method="POST" class="d-inline" onsubmit="return confirm('Bạn có chắc muốn xóa giảng viên này?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-danger btn-action"><i class="fas fa-trash-alt me-1"></i> Xóa</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
