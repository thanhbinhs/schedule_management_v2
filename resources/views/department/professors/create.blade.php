...
<div class="container mt-4">
    <h2 class="text-center mb-4">Thêm mới giảng viên</h2>

    @if ($errors->any())
        <div class="alert alert-danger">
            <strong><i class="fas fa-exclamation-triangle me-2"></i> Đã xảy ra lỗi!</strong> Hãy kiểm tra lại thông tin bạn nhập vào.<br><br>
            <ul>
    ...
    <form action="{{ route('department.professors.store') }}" method="POST">
        @csrf

        <div class="form-group">
            <label for="ProfessorName" class="form-label"><i class="fas fa-user me-1"></i> Họ và tên</label>
            <input type="text" name="ProfessorName" class="form-control" id="ProfessorName" value="{{ old('ProfessorName') }}" required>
        </div>

        <div class="form-group mb-3">
            <label for="ProfessorPhone" class="form-label"><i class="fas fa-phone me-1"></i> Số điện thoại</label>
            <input type="text" name="ProfessorPhone" class="form-control" id="ProfessorPhone" value="{{ old('ProfessorPhone') }}" required>
        </div>

        <button type="submit" class="btn btn-success"><i class="fas fa-plus-circle me-2"></i> Thêm mới giảng viên</button>
