...
@section('content')
    <div class="container">
        <h2 class="text-center">Sửa Thời Khóa Biểu</h2>

        @if ($errors->any())
            <div class="alert alert-danger">
                <strong><i class="fas fa-exclamation-triangle me-2"></i> Lỗi!</strong> Hãy kiểm tra lại dữ liệu nhập vào.<br><br>
                <ul>
        ...
        <form action="{{ route('pdt.schedules.update', $schedule) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="form-group">
                <label for="RoomID"><i class="fas fa-door-open me-1"></i> Phòng Học</label>
            ...
            <div class="form-group">
                <label for="date"><i class="far fa-calendar-alt me-1"></i> Ngày</label>
                <input type="date" name="date" id="date" class="form-control" value="{{ old('date', $schedule->date) }}" required>
            </div>

            <div class="form-group">
                <label for="session_number"><i class="fas fa-list-ol me-1"></i> Ca Học</label>
                <select name="session_number" id="session_number" class="form-control" required>
                    ...
                    @foreach ($sessions as $number => $time)
                        <option value="{{ $number }}" {{ (old('session_number', $schedule->session_number) == $number) ? 'selected' : '' }}>
                            {{ $number }} ({{ $time }})
                        </option>
                    @endforeach
                </select>
            ...
            <h4><i class="fas fa-book me-1"></i> Môn Học và Giáo Viên</h4>
            <div class="form-group">
                <label for="subject_id"><i class="fas fa-book-open me-1"></i> Môn Học</label>
                <select name="subject_id" id="subject_id" class="form-control">
                    <option value="" selected>-- Không có môn học --</option>
                    @foreach ($subjects as $subject)
                        <option value="{{ $subject->SubjectID }}" {{ (old('subject_id', $schedule->subject_id) == $subject->SubjectID) ? 'selected' : '' }}>
                            {{ $subject->SubjectName }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="professor_id"><i class="fas fa-chalkboard-teacher me-1"></i> Giáo Viên</label>
                <select name="professor_id" id="professor_id" class="form-control">
                    <option value="" selected>-- Không có giáo viên --</option>
                    @foreach ($professors as $professor)
                        <option value="{{ $professor->ProfessorID }}" {{ (old('professor_id', $schedule->professor_id) == $professor->ProfessorID) ? 'selected' : '' }}>
            ...
            <button type="submit" class="btn btn-primary mt-3"><i class="fas fa-save me-2"></i> Cập Nhật</button>
            <a href="{{ route('pdt.schedules.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left me-2"></i> Quay Lại</a>
        </form>
    </div>
@endsection